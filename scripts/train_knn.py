"""
Clasificador KNN sobre los embeddings de CNN14 para comparar con la MLP
"""
...
def plot_confusion(conf, n_classes, label_names):
    ...
    plt.ylabel("real")
    plt.tight_layout()
    plt.savefig("./checkpoints/confusion_matrix_knn.png", dpi=150)
    plt.show()
...
def run():
    ...
    # Entrenamos KNN, con embeddings de alta dimensionalidad funciona bien con distancia coseno y un numero moderado de vecinos
    print("entrenando KNN...")
    knn = KNeighborsClassifier(n_neighbors=5, metric="cosine", weights="distance", n_jobs=-1)
    knn.fit(X_train, y_train_raw)
